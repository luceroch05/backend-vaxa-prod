...
export async function updateLogo(req: Request, res: Response): Promise<void> {
  const { nombre, imagen_logo } = req.body ?? {};
  try {
    const logo = await logoService.update(tid(req), Number(req.params.id), { nombre, imagen_logo }, uid(req));
    if (!logo) { res.status(404).json({ error: 'Logo no encontrado' }); return; }
    res.json(logo);
  } catch (e) {
